...
Built entirely by coding agents as the capstone for an agentic AI coding course. Full specification: [`planning/PLAN.md`](planning/PLAN.md).

## Status

Work in progress. The market data subsystem (GBM simulator, Massive API client, SSE streaming, in-memory price cache) is implemented in `backend/app/market/` — see [`planning/MARKET_DATA_SUMMARY.md`](planning/MARKET_DATA_SUMMARY.md). Portfolio, watchlist, chat, and frontend are still to be built.
...
- **Frontend** (planned) — Next.js static export, Tailwind, Lightweight Charts
- **AI** (planned) — LiteLLM via OpenRouter (Cerebras inference), structured outputs
- **Market data** — GBM simulator by default; Massive (Polygon.io) REST API if `MASSIVE_API_KEY` is set
